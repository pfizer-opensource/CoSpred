...
def prediction_transformer(data, d_spectra, flag_fullspectrum=True, flag_evaluate=False):
    ...
    if torch.cuda.is_available():
        ...
        d_spectra["model"] = torch.nn.DataParallel(
            d_spectra["model"]).to(device)
    x_tr = x_tr.to(device)
...
def predict(predict_input, predict_dir, predict_format, testdata,
    ...
    if flag_prosit is True:
        pred = prediction_prosit(
            data, d_spectra, flag_fullspectrum, flag_evaluate)
    else:
...
def main():
    ...
    if not os.path.isfile(predict_input):
        ...
        annotation_results = annotateMGF_wSeq(
            usimgffile, testpeptides, temp_dir)
    ...
    if predict_format == 'maxquant':
        # Maxquant output
        predict(predict_input, predict_dir, 'maxquant', testdata,
                args.prosit, args.full, args.evaluate)
    elif predict_format == 'msp':
        # MSP output
        predict(predict_input, predict_dir, 'msp', testdata,
                args.prosit, args.full, args.evaluate)
    else:
