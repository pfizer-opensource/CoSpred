...
def get_intensity_target(df):
    def calc_mass_int_from_hdf5(row):
        ...
        #masses = ';'.join(map(str, row.mz))
        #intensities = ';'.join(map(str, row.intensities))
        #masses = row.mz.map(lambda x: ast.literal_eval(x))[:].tolist()
        #intensities = row.intensities.map(lambda x: ast.literal_eval(x))[:].tolist()
        #masses = row.mz.tolist()
        #intensities = row.intensities.tolist()
        #print(masses)
        lstintensities = row.intensities.split(";")
        max_val = max(lstintensities)
        lstmasses = row.mz.split(";")
        massbin=dict()
        for x in range(1600):
            massbin[x] = 0.0
        #print(massbin.keys())
        ...
        for index, ion in enumerate(lstmasses):
            if math.floor(float(ion)) in massbin.keys():
                massbin[math.floor(float(ion))] = float(lstintensities[index])/float(max_val)
        values_list = list(massbin.values())
        return [values_list]
