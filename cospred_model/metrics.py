...
from sklearn.metrics import precision_recall_curve, average_precision_score, precision_score, PrecisionRecallDisplay
...
def pearson_corr(pred, true):
    ...
    true_normalize = true / true_norm
    pred_normalize = pred / pred_norm
    corr = torch.sum((pred_normalize * true_normalize),dim=-1) / torch.sqrt(torch.sum((pred_normalize ** 2),dim=-1) * torch.sum((true_normalize ** 2),dim=-1))
    return corr
...
def roc_auc(pred_binary_arr, true_binary_arr):
    roc_auc = roc_auc_score(true_binary_arr, pred_binary_arr, multi_class="ovr", average="micro")
    # summarize scores
...
class ComputeMetrics:
    def __init__(self, true, pred, seq=None, charge=None, ce=None,
        ...
        self._tp = torch.sum((pred_binary == 1.0) & (true_binary == 1.0), dim=1).float()  # True positive
        self._tn = torch.sum((pred_binary == 0.0) & (true_binary == 0.0), dim=1).float()  # True negative
        self._fp = torch.sum((pred_binary == 1.0) & (true_binary == 0.0), dim=1).float()  # False positive
        self._fn = torch.sum((pred_binary == 0.0) & (true_binary == 1.0), dim=1).float()  # False negative
    ...
    def plot_PRcurve_micro(self, plot_dir):
        ...
        average_precision["micro"] = average_precision_score(true_binary_nonzero, pred_binary_nonzero, average="micro")

        display = PrecisionRecallDisplay(
            recall=recall["micro"],
            precision=precision["micro"],
            average_precision=average_precision["micro"],
            prevalence_pos_label=Counter(true_binary_nonzero.ravel())[1] / true_binary_nonzero.size,
        )
        display.plot(plot_chance_level=True)
        _ = display.ax_.set_title("Micro-averaged Precision-Recall Charateristic")
    ...
    def plot_PRcurve_sample(self, plot_dir):
        ...
        for i in range(true_binary.shape[0]):
            precision[i], recall[i], _ = precision_recall_curve(true_binary[i,:], pred_binary[i,:])
    ...
    def plot_ROCcurve_macro(self, plot_dir):
        ...
        for i in range(y_true.shape[0]):
            if np.any(y_true[i, :] > 0):
                ...
                if i % 100 == 0:
                    print('i = {}, tpr= {}, fpr = {}'.format(i, tpr[i], fpr[i]))
            else:
                fpr[i] = np.array([0])
                tpr[i] = np.array([1])
                auclist[i] = 1
        ...
        print(f"Macro-averaged One-vs-Rest ROC AUC score={auclist['macro']:.3f}")
